...
describe('VeiculoPrismaRepository', () => {
  ...
  it('findAll chama prisma.veiculo.findMany e retorna lista', async () => {
    ...
    expect(mockPrisma.veiculo.findMany).toHaveBeenCalledWith({ include: { cliente: true } });
  });
  ...
  it('findById chama prisma.veiculo.findUnique com id', async () => {
    ...
    expect(mockPrisma.veiculo.findUnique).toHaveBeenCalledWith({ where: { id: 'v1' }, include: { cliente: true } });
  });
  ...
  it('findByClienteId chama prisma.veiculo.findMany com clienteId', async () => {
    ...
    expect(mockPrisma.veiculo.findMany).toHaveBeenCalledWith({ where: { clienteId: 'c1' }, include: { cliente: true } });
  });
  ...
  it('update chama prisma.veiculo.update com id e data', async () => {
    ...
    expect(mockPrisma.veiculo.update).toHaveBeenCalledWith({ where: { id: 'v1' }, data: dto });
  });
  ...
  it('delete chama prisma.veiculo.delete com id', async () => {
    ...
    expect(mockPrisma.veiculo.delete).toHaveBeenCalledWith({ where: { id: 'v1' } });
  });
});
