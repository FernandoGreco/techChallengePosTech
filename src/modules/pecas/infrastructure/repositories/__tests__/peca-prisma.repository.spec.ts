...
describe('PecaPrismaRepository', () => {
  ...
  it('findById chama prisma.peca.findUnique com id', async () => {
    ...
    expect(mockPrisma.peca.findUnique).toHaveBeenCalledWith({ where: { id: 'p1' } });
  });
  ...
  it('update chama prisma.peca.update com id e data', async () => {
    ...
    expect(mockPrisma.peca.update).toHaveBeenCalledWith({ where: { id: 'p1' }, data: dto });
  });
  ...
  it('delete chama prisma.peca.delete com id', async () => {
    ...
    expect(mockPrisma.peca.delete).toHaveBeenCalledWith({ where: { id: 'p1' } });
  });
});
