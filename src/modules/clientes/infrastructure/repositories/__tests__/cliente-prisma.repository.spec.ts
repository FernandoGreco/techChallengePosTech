...
describe('ClientePrismaRepository', () => {
  ...
  it('findAll chama prisma.cliente.findMany e retorna lista', async () => {
    ...
    expect(mockPrisma.cliente.findMany).toHaveBeenCalledWith({ include: { veiculos: true } });
  });
  ...
  it('findById chama prisma.cliente.findUnique com where id', async () => {
    ...
    expect(mockPrisma.cliente.findUnique).toHaveBeenCalledWith({ where: { id: '1' }, include: { veiculos: true } });
  });
  ...
  it('findByDocumento chama prisma.cliente.findUnique com where documento', async () => {
    ...
    expect(mockPrisma.cliente.findUnique).toHaveBeenCalledWith({ where: { documento: 'doc' }, include: { veiculos: true } });
  });
  ...
  it('update chama prisma.cliente.update com id e data', async () => {
    ...
    expect(mockPrisma.cliente.update).toHaveBeenCalledWith({ where: { id: '1' }, data: dto });
  });
  ...
  it('delete chama prisma.cliente.delete com id', async () => {
    ...
    expect(mockPrisma.cliente.delete).toHaveBeenCalledWith({ where: { id: '1' } });
  });
});
